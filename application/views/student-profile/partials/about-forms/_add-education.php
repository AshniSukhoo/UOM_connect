<form method="post" action="<?=base_url()?>user-actions/save-new-education" autocomplete="off">
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4><i class="fa fa-suitcase"></i> Adding Education</h4>
		</div><!--/.panel-heading-->
				...
				<div class="col-md-12">
					<div class="form-group<?=($this->keeper->has('institution_name_error'))?' has-error':''?>">
						<div class="input-group">
							...
							<input type="text" name="institution_name" placeholder="Enter the name of the institution attended" class="form-control" value="<?=$this->keeper->has('institution_name')?$this->keeper->get('institution_name'):''?>" />
						</div><!--/.input=-group--->
					...
					<div class="form-group<?=($this->keeper->has('major_error'))?' has-error':''?>">
						<div class="input-group">
							...
							<input type="text" name="major" placeholder="Enter the major subject studied or degree" class="form-control" value="<?=$this->keeper->has('major')?$this->keeper->get('major'):''?>" />
						</div><!--/.input=-group--->
					...
					<div class="form-group<?=($this->keeper->has('year_joined_error'))?' has-error':''?>">
						<select name="year_joined" id="year_joined" class="form-control">
							...
							<?php foreach(range(date('Y'), date('Y') - 100 ) as $year): ?>
								<option value="<?=$year?>" <?=($this->keeper->has('year_joined_value') && $this->keeper->get('year_joined_value') == $year)?'selected':''?>><?=$year?></option>
							<?php endforeach; ?>
						</select>
					...
					<div class="form-group<?=($this->keeper->has('year_left_error'))?' has-error':''?>">
						<select name="year_left" id="year_left" class="form-control">
							...
							<?php foreach(range(date('Y'), date('Y') - 100 ) as $year): ?>
								<option value="<?=$year?>" <?=($this->keeper->has('year_left_value') && $this->keeper->get('year_left_value') == $year)?'selected':''?>><?=$year?></option>
							<?php endforeach; ?>
						</select>
